...
export function HeatmapGrid({ wards }: HeatmapGridProps) {
  if (!wards.length) {
    return (
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      {wards.map((ward) => (
        <article
          key={ward.ward_id}
          className={`rounded-lg border p-4 transition-all duration-200 hover:shadow-md ${riskClasses(
            ward.risk_level || ward.status
          )}`}
        >
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                <MapPin size={17} />
                <h3 className="font-semibold text-slate-900">{ward.ward_name}</h3>
              </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              ...
              <p className="text-xl font-bold">{Math.round(ward.compliance_score ?? 100)}%</p>
            </div>
          </div>
        </article>
      ))}
