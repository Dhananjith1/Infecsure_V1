...
| **2026-09-19 23:30** | Permanent Backend Startup Settings Fix: Resolved Pydantic `ValidationError` extra_forbidden errors on `SEED_*_PASSWORD` keys by adding `extra="ignore"` to `SettingsConfigDict` and declaring explicit `seed_*_password` fields on `Settings` model in `app/config.py`. Updated `auth_service.py` to check `settings` first with fallback to `os.environ`. | `app/config.py`, `app/services/auth_service.py`, `SYSTEM_DOCUMENTATION_AND_ROADMAP.md` | ✅ Fixed & Verified |
| **2026-09-19 23:31** | Seed Password Credentials Alignment: Updated `.env` seed password variables (`SEED_SISTER_PASSWORD=sister@123`, `SEED_LAB_PASSWORD=lab@123`, `SEED_DOCTOR_PASSWORD=doctor@123`, `SEED_STAFF_PASSWORD=staff@123`) to match exact default user credentials table. | `.env`, `SYSTEM_DOCUMENTATION_AND_ROADMAP.md` | ✅ Synced & Verified |
| **2026-09-19 23:49** | Frontend Dependency & Build Error Fix: Resolved Vite `ERR_MODULE_NOT_FOUND` error (`tinyglobby/dist/index.mjs`) by performing a clean `node_modules` reinstall in `InfecSure_Frontend/` and aligning `vite` to latest stable release. Added `doctor_instructions?: string;` property to `AlertItem` in `src/types.ts` to resolve TypeScript compilation errors. Verified 100% clean production bundle build (`npm run build`). | `InfecSure_Frontend/src/types.ts`, `InfecSure_Frontend/package.json`, `SYSTEM_DOCUMENTATION_AND_ROADMAP.md` | ✅ Cleaned, Fixed & Built |
