...
| **2026-09-20 16:10** | Lab Slip OCR NumPy Serialization & Auto-fill Fix: (1) Resolved `PydanticSerializationError: Unable to serialize unknown type: <class 'numpy.bool'>` by casting token `needs_review` flags to native Python booleans and implementing `_sanitize_for_json()` in `ocr_service.py` to recursively sanitize NumPy scalars (`np.generic`) and NumPy ndarrays into standard JSON-serializable Python types. (2) Enhanced lab slip field extraction in `lab_results.py` to detect patient names with colons/punctuation (e.g. `Mr: Kasun Perera`), BHT/OPD reference numbers, and test results from unstructured OCR text. (3) Updated `Dashboard.tsx` in frontend to fallback to `patient_name` for `bht_number` when BHT is unstated. Verified with full test suite passing (39/39 tests). | `app/services/ocr_service.py`, `app/routers/lab_results.py`, `InfecSure_Frontend/src/pages/Lab/Dashboard.tsx`, `SYSTEM_DOCUMENTATION_AND_ROADMAP.md` | ✅ Applied & Verified |
| **2026-09-20 17:55** | Lab Section Fix — Submission Status Tracker & Pure EasyOCR/OpenCV Pipeline: (1) Fixed Submission Status Tracker not displaying recent lab submissions by updating `list_lab_results()` in `firebase_service.py` and `lab_results.py` to filter directly by `entered_by_uid` in Firestore queries, ensuring the authenticated lab user's recent submissions are retrieved rather than truncated by global collection limits. (2) Completely removed Google Cloud Vision API calls and client setup from `ocr_service.py` and `lab_results.py`, migrating strictly to an optimized single-pass OpenCV + EasyOCR pipeline (`_preprocess_image` with dimension normalization, deskew, and CLAHE contrast enhancement). Switched EasyOCR to `greedy` decoding to reduce execution time from ~78s to ~3s on CPU. (3) Increased frontend Axios timeout from 60s to 120s in `client.ts` to prevent request timeouts on large photo scans. Verified with test suite (39/39 passed). | `app/services/ocr_service.py`, `app/routers/lab_results.py`, `app/services/firebase_service.py`, `InfecSure_Frontend/src/api/client.ts`, `SYSTEM_DOCUMENTATION_AND_ROADMAP.md` | ✅ Applied & Verified |
| **2026-09-21 12:13** | Software Testing & Quality Assurance Documentation: Drafted complete functional software test case tables (TC-AUTH, TC-AUD, TC-LAB, TC-ML, TC-ALT, TC-MAP) covering 6 core modules with test preconditions, steps, expected results, actual results, and status. Generated official styled Microsoft Word document report (`docs/InfecSure_Functional_Test_Cases_Report.docx`) for final academic thesis/report inclusion. | `scripts/generate_test_report_docx.py`, `docs/InfecSure_Functional_Test_Cases_Report.docx`, `SYSTEM_DOCUMENTATION_AND_ROADMAP.md` | ✅ Generated & Documented |
| **2026-09-21 12:40** | Matron/Sister Account Credential Fix: Reset and permanently synced Firebase Authentication password for `matron@infecsure.com` to `sister@123` via Firebase Admin SDK (`auth.update_user`). Verified Firestore user profile (`role: "sister"`) and tested live authentication against both Google Identity Toolkit REST API and FastAPI `/auth/login` endpoint (HTTP 200 / Bearer token issued). | `scripts/fix_matron_account.py`, `SYSTEM_DOCUMENTATION_AND_ROADMAP.md` | ✅ 100% Verified Live |
| **2026-09-21 12:42** | Documentation Audit & Synchronization: Audited and synchronized `README.md` and `SYSTEM_DOCUMENTATION_AND_ROADMAP.md`. Enforced consistent Default Login Credentials tables across both files with exact roles (`Nursing Sister (Matron)`), credentials (`matron@infecsure.com` / `sister@123`), and target dashboard paths. | `README.md`, `SYSTEM_DOCUMENTATION_AND_ROADMAP.md` | ✅ Audited & Synchronized |
| **2026-09-21 13:00** | Cross-Dashboard Hospital Heatmap Unification & Interactive SVG Floor Plan: (1) Fixed data divergence between `/heatmap/` and `/public/heatmap` by standardizing `app/routers/public.py` to return consistent schema, real-time risk scores, compliance scores, and anomaly counts across all 6 wards. (2) Standardized all 3 user role dashboards (Staff in `NoticePanel.tsx` & `HeatmapView.tsx`, ICNO in `Dashboard.tsx` & `Surveillance.tsx`, and Nursing Sister in `SisterDashboard.tsx`) to consume the identical `getHeatmapWithFallback()` data pipeline and eliminated dual-fetch race conditions. (3) Completely upgraded `HeatmapGrid.tsx` into a unified component featuring an interactive architectural SVG Hospital Floor Plan of Divisional Hospital Thalangama (ETU, Male, Female, OPD, Family Clinic, Psychiatry) with real-time Green/Amber/Red risk fills, pulse highlights, interactive click/hover inspection, floor filters, and synchronized ward risk cards. Verified with automated test suite (39/39 tests passing). | `app/routers/public.py`, `InfecSure_Frontend/src/components/HeatmapGrid.tsx`, `InfecSure_Frontend/src/pages/Public/NoticePanel.tsx`, `InfecSure_Frontend/src/pages/Public/HeatmapView.tsx`, `InfecSure_Frontend/src/pages/ICNO/Dashboard.tsx`, `InfecSure_Frontend/src/pages/ICNO/Surveillance.tsx`, `SYSTEM_DOCUMENTATION_AND_ROADMAP.md` | ✅ 100% Fixed & Verified |
| **2026-09-21 13:08** | Heatmap UI Simplification: Restored the normal previous clean card grid layout in `HeatmapGrid.tsx` upon user request, removing the dark SVG floor plan and filter options bar. Preserved cross-dashboard data synchronization across Staff, ICNO, and Sister dashboards so all 3 views display the exact same live risk scores, compliance percentages, and ward cards. | `InfecSure_Frontend/src/components/HeatmapGrid.tsx`, `SYSTEM_DOCUMENTATION_AND_ROADMAP.md` | ✅ Restored & Synchronized |
