...
| **2026-09-20 18:12** | Lab Dashboard Camera Live Preview Black Screen Fix: Resolved issue where camera stream was active but `<video>` rendered a black box in `Dashboard.tsx`. (1) Fixed asynchronous mount race condition by moving `video.srcObject = cameraStream` binding and playback into a dedicated React `useEffect` with `loadedmetadata` and `loadeddata` event listeners. (2) Added `autoPlay`, `playsInline`, and `muted` attributes to the `<video>` element to satisfy browser autoplay policies. (3) Added fallback constraints to `getUserMedia()` for standard webcams (`{ audio: false, video: true }`) if `facingMode: "environment"` is unsupported on desktop/laptop hardware. (4) Added frame readiness check before snapshot capture. | `InfecSure_Frontend/src/pages/Lab/Dashboard.tsx`, `SYSTEM_DOCUMENTATION_AND_ROADMAP.md` | ✅ Applied & Verified |
